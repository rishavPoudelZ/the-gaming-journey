...
public class AddGameService {
    ...
    public void saveGameWithImages(HttpServletRequest request, String gameTitle, List<Part> imageParts, String uploadPath, String[] selectedGenres, String[] selectedPlatforms)
        ...
        try (Connection conn = DbConfig.getDbConnection()) {
            String sql = "INSERT INTO games (title, description, developer, releaseDate, DateUploaded, imagesUrl, age_restriction, GameUrl) " +
                         "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            int gameId;
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                stmt.setString(6, imagePathsString);
         ...
         // Link to genres
            if (selectedGenres != null) {
                String genreSql = "INSERT INTO genres_games (gameId, genreId) VALUES (?, (SELECT id FROM genres WHERE name = ?))";
         ...
         // Link to platforms
            if (selectedPlatforms != null) {
                String platformSql = "INSERT INTO platforms_games (gameId, platformId) VALUES (?, (SELECT id FROM platforms WHERE name = ?))";
